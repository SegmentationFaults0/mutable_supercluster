...
const compareFn = (a, b) =>
  a.geometry[0][0] - b.geometry[0][0] ||
  a.geometry[0][1] - b.geometry[0][1] ||
  a.geometry[0][2] - b.geometry[0][2] ||
  a.geometry[0][3] - b.geometry[0][3];

const places = JSON.parse(
  readFileSync(new URL("./fixtures/places.json", import.meta.url)),
);
const placesTile = JSON.parse(
  readFileSync(new URL("./fixtures/places-z0-0-0.json", import.meta.url)),
);
const sortedTileFeatures = placesTile.features.sort(compareFn);
const placesTileMin5 = JSON.parse(
  readFileSync(new URL("./fixtures/places-z0-0-0-min5.json", import.meta.url)),
);
...
test("supports minPoints option", () => {
  const index = new Supercluster({ minPoints: 5, getId }).load(
    structuredClone(places.features),
  );
  const tile = index.getTile(0, 0, 0);
  tile.features.sort(compareFn);
...
test("generates unique ids with generateId option", () => {
  const index = new Supercluster({ generateId: true, getId }).load(
    structuredClone(places.features),
  );
  const ids = index
    .getTile(0, 0, 0)
  ...
  assert.deepEqual(
    ids,
    [62, 24, 22, 12, 28, 20, 125, 119, 30, 118, 81, 21, 81, 118],
  );
});

test("getLeaves handles null-property features", () => {
  const index = new Supercluster({ getId }).load(
    structuredClone(places.features).concat([
      {
        type: "Feature",
        properties: null,
        geometry: {
